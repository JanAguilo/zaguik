...
def run_one_L(
    ...
    proof_result = generate_proof(
        ...
        proof_filename=f"proof_trial_{trial_id}.json",
        max_len=L,
    )
...
def main() -> None:
    ...
    parser.add_argument("--repetitions", type=int, default=DEFAULT_REPETITIONS, help=f"Number of repetitions per L (default: {DEFAULT_REPETITIONS})")
    ...
    # Run each L for N repetitions
    results = []
    for trial_id in range(1, args.repetitions + 1):
        logger.info("=== Trial %d/%d ===", trial_id, args.repetitions)
        for L in sorted(args.lengths):
            logger.info("--- Trial %d | L = %d ---", trial_id, L)
            res = run_one_L(
                L,
                trial_id,
                reference_output,
            ...
            results.append(res)
            print(f"  trial={trial_id} L={L} truncated={res['truncated']} pii_passed={res['pii_passed']} proof_generated={res['proof_generated']} proof_verified={res.get('proof_verified')}")
