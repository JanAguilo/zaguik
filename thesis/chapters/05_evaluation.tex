...
Experiments were run sequentially, with no parallel execution across configurations. We evaluated five encoding lengths, $L \in \{128, 256, 512, 1024, 2048\}$ bytes, each repeated fifteen times ($n = 15$) using independently generated reference outputs from the same prompt and model. Each prompt was designed to produce an output exceeding 2048 bytes, so that for every value of $L$ the encoding window was fully populated; when $L < 2048$, only the prefix of length $L$ bytes was encoded and attested. This yields $5 \times 15 = 75$ proof-generation and verification attempts in total. The mean values and sample statistics over the fifteen runs are reported below.
...
Table~\ref{tab:results} presents the proof size, proof generation time, witness computation time, and verification time for each value of $L$, reported as mean $\pm$ sample standard deviation over the fifteen runs. All 75 proof-generation and verification attempts succeeded across the five encoding lengths.
