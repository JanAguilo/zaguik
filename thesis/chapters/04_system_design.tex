...
\textbf{Security model.} We assume the verifier is honest and follows the verification algorithm, and we are careful to distinguish two parts of the guarantee with respect to a potentially malicious prover, since they do not enjoy the same protection. The zk-SNARK component is sound against a malicious prover: by the soundness of zk-SNARKs (Definition~\ref{def:completeness-soundness}), no probabilistic polynomial-time adversary can produce a valid proof for $C(x) = 0$ without knowledge of a witness $x$ satisfying that relation, and zero-knowledge (Definition~\ref{def:czk}) ensures the proof reveals nothing about $x$ beyond the truth of the statement. The binding between that witness and a specific model output is a separate matter. Because the commitment $h = H(y_L)$ is computed and published \emph{outside} the circuit, its correspondence to the witness $x$ is not enforced cryptographically; it holds only under an \emph{honest-prover assumption}. A malicious prover could therefore prove $C(x) = 0$ for some innocuous $x$ while publishing a hash derived from an unrelated output. This is the principal limitation of the present design, and it is worth stating plainly: the circuit statement is verifiable without trust, but the link from the proof to the actual delivered output is not, which falls short of the fully untrusted-prover setting that motivates this work (Chapter~\ref{chap:introduction}). Closing this gap by verifying the hash \emph{inside} the circuit, ideally with a SNARK-friendly hash function, is the most important direction we leave to future work (Chapter~\ref{chap:conclusions}).
...
To make the pipeline concrete, we trace two short strings end to end: one that satisfies the byte-level policy and one that does not. The strings are kept short so that every byte can be inspected by hand; the mechanism is identical for longer LLM outputs, only the padding length differs.

\medskip

\textbf{Example A: a clean response (PASS).}
Suppose the LLM answers a general-knowledge query with the response:

\begin{quote}
\texttt{The capital of France is Paris}
\end{quote}

This string contains 30 UTF-8 bytes.  Examining each byte value, none of them is suspicious: there are no \texttt{@} characters, no digits, no hyphens, and no periods.  The encoding $x = \mathrm{Enc}_L(y_L)$ is formed by appending $L - 30$ zero bytes and dividing each entry by~255, giving a float vector of shape $[1, L]$.  The byte-level circuit evaluates the sum over all positions and returns $o = 0$, which signals \texttt{PASS}.

The prover computes the commitment $h = \text{SHA-256}(y_L)$ over the zero-padded raw bytes and publishes it together with the proof~$\pi$.  The verifier runs the verification algorithm with $(\pi, \mathrm{vk}, o = 0, h)$ and accepts: a clean LLM response to a general-knowledge question has been cryptographically attested, with neither the response text nor the filter logic ever leaving the prover.

\medskip

\textbf{Example B: a response containing PII (FAIL).}
Now suppose the LLM produces a response that inadvertently reveals a contact detail:

\begin{quote}
\texttt{For further assistance please contact support@helpdesk.org}
\end{quote}

This string contains 57 UTF-8 bytes.  Two suspicious bytes appear: the \texttt{@} character (byte value~64) between \texttt{support} and \texttt{helpdesk}, and the period (byte~46) separating \texttt{helpdesk} from \texttt{org}.  After the same encoding procedure the circuit counts both values and returns $o = 2$, signalling \texttt{FAIL}.  In a real deployment this output would already have been blocked by the operational regex detector in the filtering phase, since the email address matches a standard pattern; the circuit then corroborates that finding from first principles at the byte level.

The proof~$\pi$ is still generated and is still valid: it attests that the circuit was correctly executed on the encoded string and returned the count $o = 2$.  The verifier accepts the proof as a correct execution of the certified circuit, but observes $o > 0$ and concludes that the policy was not satisfied.  Importantly, the verifier learns only that the count was~2; the exact string, the position of the suspicious bytes, and any other content of the response remain private.

\medskip

Table~\ref{tab:worked-example} summarises the two cases side by side, showing what the verifier sees and what remains private.

\begin{table}[htbp]
\centering
\caption{Public and private information for the two worked examples.}
\label{tab:worked-example}
\footnotesize
\begin{tabular}{l l l}
\hline
\hline
 & Example A (PASS) & Example B (FAIL) \\
\hline
Response text $y$ & \emph{private} & \emph{private} \\
Suspicious-byte count $o$ & $0$ (public) & $2$ (public) \\
Hash commitment $h$ & SHA-256$(y_L)$ (public) & SHA-256$(y_L)$ (public) \\
Proof $\pi$ & valid & valid \\
Verifier verdict & policy satisfied & policy not satisfied \\
\hline
\hline
\end{tabular}
\end{table}
