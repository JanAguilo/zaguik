...
Els reguladors exigeixen cada cop més garanties basades en evidències sobre com els sistemes d'intel·ligència artificial tracten la informació sensible. Presentem Zaguik, un sistema d'atestació que preserva la privadesa i demostra, en coneixement zero, que la resposta d'un gran model de llenguatge (LLM) ha superat un filtre d'informació d'identificació personal (PII), sense revelar el text ni el model de filtratge. El sistema genera la resposta, aplica una política de detecció de PII i produeix un zk-SNARK amb EzKL sobre la inferència del detector. La prova només exposa un compromís públic (un hash) del prefix de sortida atestat, tot mantenint-ne privat el contingut. Proporcionem una implementació de referència executable, n'analitzem els requisits de configuració i les restriccions de compatibilitat d'ONNX, i mostrem com aquestes proves poden donar suport a l'auditoria alineada amb el GDPR i a una governança de la IA transparent.
...
zk-SNARK using EzKL over the detector inference. The proof exposes only a public commitment (a hash) to the attested output prefix, binding the proof to that prefix at the protocol level while keeping its contents private. We provide a runnable reference
...
Los reguladores exigen cada vez más garantías basadas en evidencias sobre cómo los sistemas de inteligencia artificial tratan la información sensible. Presentamos Zaguik, un sistema de atestación que preserva la privacidad y demuestra, en conocimiento cero, que la respuesta de un gran modelo de lenguaje (LLM) ha superado un filtro de información de identificación personal (PII), sin revelar el texto ni el modelo de filtrado. El sistema genera la respuesta, aplica una política de detección de PII y produce un zk-SNARK con EzKL sobre la inferencia del detector. La prueba sólo expone un compromiso público (un hash) del prefijo de salida atestado, manteniendo privado su contenido. Proporcionamos una implementación de referencia ejecutable, analizamos los requisitos de configuración y las restricciones de compatibilidad de ONNX, y mostramos cómo estas pruebas pueden apoyar la auditoría alineada con el GDPR y una gobernanza de la IA transparente.
